...
import {View, Text, TouchableOpacity, Image, StyleSheet, ViewProps, Pressable} from 'react-native';
import FastImage from 'react-native-fast-image';
import { AutoScaledFont, getPrimaryTextInputSize } from '../config/Size';
import { Constants } from '../constants/Constants';
import { Colors } from '../constants/Colors';
import { useSelector } from 'react-redux';
import { ArrowLeft, Bell, DotsThreeVertical, MagnifyingGlass } from 'phosphor-react-native';
...
const SettingsSections = ({name, onpressSection,showBottomLine=true,icon }: SettingsSectionsProps) => {
  // useSelector(
  //   (state: any) => state.piData.piData.profileDownloadURL || null,
  // );

  return (
    <Pressable onPress={onpressSection}>
    <View
      style={[
      ...
      {showBottomLine && 
      <View style={{height:AutoScaledFont(1),backgroundColor:Colors.lightgray,marginTop:AutoScaledFont(6)}}>
      </View>
              }
